...
func (h *searchComp) Render() app.UI {
	...
	return app.Div().Body(
				...
				app.Td().Body(
					app.Div().Style("margin-left", "60px").Body(
						...
						app.Div().Title("Der einzige Algorithmus ohne Ähnlichkeitssuche sondern klassisch nach einer festen Buchstabenkombination.").Body(
							...
							app.Text("Klassisch"),
							app.Text(" ----> sucht buchstabengenau nach dem Suchtext innerhalb des Namens. Extrem schnell."),
						),
					),
				),
			),
		...
		app.Br(),
		app.Text("Manche Suchen (Soundex mit nur einem Wort) können die Suche manchmal abstürzen lassen (Seite bleibt hängen, Button reagiert nicht mehr) => in diesem Fall muss die Seite im Browser neu geladen werden."),
		app.Br(),
		...
		app.Text(" => als Bps. für den Nachnamen 'Vitzthum von Eckstädt' mit beliebigen Vornamen. (Mehrteilige Nachnamen wurden im Index (außer in Auerbach und Torgau-Delitzsch) verkürzt und bestehen immer nur aus einem Wort)"),
		app.Br(),
		app.B().Text("* vonSchönberg"),
		app.Text(" => Nur im Gebiet Torgau-Delitzsch und Auerbach sind adelige Nachnamen genauer erfasst, müssen aber ohne Leerzeichen geschrieben werden, damit die Suche damit funktioniert."),
		app.Br(),
		app.Br(),
