...
func (h *searchComp) Render() app.UI {
	...
	return app.Div().Body(
			...
			app.Tr().Body(
				app.Td().Body(
					app.Div().Body(
						...
						app.Label().Text("Bisher erfasst: Dresden Meißen Freiberg Dippoldiswalde Auerbach Torgau (+Teile von Eilenburg und Bautzen)"),
						app.Br(),
						app.Br(),
		...
		app.Text(" => Sucht nach Nachname Schönberg mit beliebigen Vornamen. (Adelige Nachnamen z.B. 'von Schönberg' sind im Index ebenfalls als 'Schönberg' geführt und können (außer in Torgau, Delitzsch und Auerbach) nicht von der nicht-adeligen Variante unterschieden werden)"),
		app.Br(),
		...
		app.Text(" => Nur im Gebiet Torgau, Torgau und Auerbach sind adelige Nachnamen genauer erfasst, müssen aber ohne Leerzeichen geschrieben werden, damit die Suche damit funktioniert."),
		app.Br(),
		app.Br(),
